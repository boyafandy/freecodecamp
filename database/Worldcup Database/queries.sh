...
echo "$($PSQL "WITH result AS (SELECT DISTINCT(unnest(array[winner_id, opponent_id])) AS team_id FROM games WHERE year = 2014 AND round='Eighth-Final') SELECT name FROM teams INNER JOIN result ON teams.team_id = result.team_id ORDER BY name;")"
